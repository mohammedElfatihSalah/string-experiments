...
class Net(hk.Module):
  ...
  def _msg_passing_step(self,
    ...
    if (not first_step) and repred and self.decode_hints:
      ...
      # if advance is 1 reset manually
      advance = decoded_hint['advance'].data
      decoded_i = decoded_hint['i'].data
    ...
    new_mp_state = _MessagePassingScanState(
        hint_preds=hint_preds, diff_logits=None, gt_diffs=None,
        output_preds=output_preds, hiddens=hiddens, prev_hint=prev_hint_)
  ...
  def __call__(self, features: _Features, repred: bool):
    ...
    mp_state = _MessagePassingScanState(
        ...
        output_preds=None, hiddens=hiddens, prev_hint =Hint(i=None, j=None, s=None))
  ...
  def _one_step_pred(
    ...
    temp = jnp.expand_dims(graph_fts, axis=1)
    temp = jnp.repeat(temp,nb_nodes, axis=1)
    z = jnp.concatenate([node_fts, temp], axis=-1)
    ...
    # mask_for_j = np.zeros((bs, nb_nodes))
    # idx = nb_nodes // 5
    # idx = nb_nodes - idx
    # mask_for_j[:,:idx]=-1e9
    # hint_preds['j'] += mask_for_j
